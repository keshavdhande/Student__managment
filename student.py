...
from matplotlib.pyplot import close    #pip install pillow for show images
...
class Student:
    def __init__(self,root):
        self.root=root
        self.root.geometry('1530x790+0+0')   #this are geometry screen size points
        ...
        img=Image.open("college_images/7th.jpg")  #for store image
        ...
        self.btn_1.place(x=0,y=0,width=540,height=160)   #this show image to window
        #code for show second image

        img_2=Image.open("college_images/5th.jpg")  #for store image
        img_2=img_2.resize((540,160),Image.ANTIALIAS)  # adjust size of images
        self.photoimg_1=ImageTk.PhotoImage(img_2)
        self.btn_2=Button(self.root,command=self.open_img_2,image=self.photoimg_1,cursor="hand2")
        self.btn_2.place(x=540,y=0,width=540,height=160)   #this show image to window
        #code for show third image

        img_3=Image.open("college_images/6th.jpg")  #for store image
        img_3=img_3.resize((540,160),Image.ANTIALIAS)  # adjust size of images
        self.photoimg_3=ImageTk.PhotoImage(img_3)
        self.btn_3=Button(self.root,command=self.open_img_3,image=self.photoimg_3,cursor="hand2")
        self.btn_3.place(x=1000,y=0,width=540,height=160)   #this show image to window


       #code for show background image
        img_4=Image.open("college_images/university.jpg")  #for store image
        img_4=img_4.resize((1530,710),Image.ANTIALIAS)  # adjust size of images
        self.photoimg_4=ImageTk.PhotoImage(img_4)
        bg_lbl=Label(self.root,image=self.photoimg_4,bd=2,relief=RIDGE)  #bd stands for border
        bg_lbl.place(x=0,y=160,width=1530,height=710) #it will show background image
        #this is title
        ...
        lbl_title=Label(bg_lbl,text="STUDENT MANAGEMENT SYSTEM",font=("Sans Serif",34),fg="blue",bg="white")      
        lbl_title.place(x=0,y=0,width=1530,height=45)
        exit_button = Button( text="Exit", command=root.quit,height=2,width=12,fg="red",font=("",9,'bold'),bg='white',bd=1,relief=SOLID,)
        ...
        #left frame
        DataLeftFrame=LabelFrame(Manage_frame,bd=4,relief=RIDGE,padx=2,text="Student Information",font=("times new roman",12,"bold"),fg="red",bg="white") #fg means font color  #you used manage here because we want to show our left frame onto the out manage frame
        DataLeftFrame.place(x=15,y=10,width=620,height=540)  

        #current course labelframe 
        std_lbl_info_frame=LabelFrame(DataLeftFrame,bd=1,relief=SOLID,padx=2,text="Current Course Information",font=("times new roman",12,"bold"),fg="green",bg="white") #fg means font color  #you used manage here because we want to show our left frame onto the out manage frame
        std_lbl_info_frame.place(x=0,y=11,width=600,height=115)  

        #Department 
        lbl_dep=Label(std_lbl_info_frame,text="Department:",font=("arial",12,"bold"),bg="white") # this is a label for department
        lbl_dep.grid(row=0,column=0,padx=2,sticky=W) #sticky west for put it on west site
        ...
        combo_dep.current(0)  #this will show by default value at 0 index i.e select department
        combo_dep.grid(row=0,column=1,padx=2,pady=10,sticky=W)  #padx stands for padding
        #course
        course_std=Label(std_lbl_info_frame,text="Courses:",font=("arial",12,"bold"),bg="white") # this is a label for department
        course_std.grid(row=0,column=2,padx=2,pady=10,sticky=W) #sticky west for put it on west site
        ...
        com_txtcourse_std["value"]=("Select Course","BCS","BCA","MCA","MCS")
        com_txtcourse_std.current(0)  #this will show by default value at 0 index i.e select department
        com_txtcourse_std.grid(row=0,column=3,padx=2,pady=10,sticky=W)  #padx stands for padding
        ...
        cpm_txt_current_year=ttk.Combobox(std_lbl_info_frame,state="readonly",textvariable=self.var_year,font=("arial",12,"bold"),width=17)
        cpm_txt_current_year['value']=("Select year","2020-2021","2021-2022","2022-2023")
        ...
        com_semester=ttk.Combobox(std_lbl_info_frame,state="readonly",textvariable=self.var_semester,font=("arial",12,"bold"),width=17)
        
        com_semester['value']=("Select Semester","sem1","sem2","sem3","sem4",'sem5',"sem6")
        com_semester.current(0)
        com_semester.grid(row=1,column=3,sticky=W,padx=2,pady=10)
  
        #student class frame
        std_lbl_class_frame=LabelFrame(DataLeftFrame,bd=0.5,relief=SOLID,padx=5,text="Student Class  Information",font=("times new roman",12,"bold"),fg="green",bg="white") #fg means font color  #you used manage here because we want to show our left frame onto the out manage frame
        std_lbl_class_frame.place(x=0,y=150,width=600,height=230)  
           #id
        label_id=Label(std_lbl_class_frame,font=("arial",12,"bold"),text="Student Id:",bg="white")
        ...
        id_entry.grid(row=0,column=1,sticky=W,padx=2,pady=7)
            #name        
        lbl_name=Label(std_lbl_class_frame,font=("arial",12,"bold"),text="Full Name:",bg="white")
        lbl_name.grid(row=0,column=2,sticky=W,padx=2,pady=7)
        txt_name=ttk.Entry(std_lbl_class_frame,textvariable=self.var_std_name,font=("arial",11,"bold"),width=20)
        ...
        com_txt_div=ttk.Combobox(std_lbl_class_frame,state="readonly",textvariable=self.var_div,font=("arial",12,"bold"),width=18)
        ...
        txt_roll=ttk.Entry(std_lbl_class_frame,textvariable=self.var_roll,font=("arial",11,"bold"),width=20)
        ...
        txt_dob=ttk.Entry(std_lbl_class_frame,width=20,textvariable=self.var_dob,font=("arial",11,"bold"))
        ...
        txt_phone=ttk.Entry(std_lbl_class_frame,width=20,textvariable=self.var_phone,font=("arial",11,"bold"))
        ...
        txt_teacher=ttk.Entry(std_lbl_class_frame,width=20,textvariable=self.var_teacher,font=("arial",11,"bold"))
  ...
  #frames for button
        ...
        #Right  frame
        DatarightFrame=LabelFrame(Manage_frame,bd=4,relief=RIDGE,padx=2,text="Student Details",font=("times new roman",12,"bold"),fg="red",bg="white") #fg means font color  #you used manage here because we want to show our left frame onto the out manage frame
        DatarightFrame.place(x=640,y=10,width=670,height=540)

        search_Frame=LabelFrame(DatarightFrame,bd=4,relief=RIDGE,padx=2,text="View Student & Search System",font=("times",12,"bold"),fg="black",bg="white") #fg means font color  #you used manage here because we want to show our left frame onto the out manage frame
        search_Frame.place(x=0,y=10,width=655,height=65)

        searcg_by=Label(search_Frame,font=("arial",11,"bold"),text="Search By:",bg="wheat",fg="red")
        searcg_by.grid(row=0,column=0,sticky=W,padx=5,pady=7)       
         
        #search
        self.var_com_search=StringVar() 
        com_txt_search=ttk.Combobox(search_Frame,state="readonly",textvariable=self.var_com_search,font=("arial",12,"bold"),width=13)
        com_txt_search['value']=("Select Option","Roll","Phone","stundnt_id")        
        ...
        btn_showAll=Button(search_Frame,text="Show All",command=self.fetch_data,font=("arial",11,"bold"),width=10,bg="blue",fg="white")
        ...
        #for heading
        self.student_table=ttk.Treeview(tabel_frame,columns=("dep","course","year","sem","id","name","div","roll","gender","dob","email","phone","address","teacher"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    def add_data(self):
        if (self.var_dep.get()=="" or self.var_email.get()=="" or self.var_std_id.get()==""):
            ...
            try:
                ...
                conn=mysql.connector.connect(host="localhost",username="root",password="root",database="studentdata")
    ...
    def update_data(self):
                if (self.var_dep.get()=="" or self.var_email.get()=="" or self.var_std_id.get()==""):
                    ...
                    try:
                        ...
                        messagebox.showinfo("Sucess","Student Successfully updated",parent=self.root)
...
#open img
    ...
    def open_img(self):
        ...
        img=Image.open(fln)
        img_browse=img.resize((540,160),Image.ANTIALIAS)  # adjust size of images
        self.photoimg_br=ImageTk.PhotoImage(img_browse)
        self.btn_1.config(image=self.photoimg_br)
    def open_img_2(self):
        fln3=filedialog.askopenfilename(initialdir=os.getcwd(),title="open Images",filetypes=(("JPG File","*.jpg"),("PNG File","*.png"),("All files","*.*")))
        img3=Image.open(fln3)
        img_browse=img3.resize((540,160),Image.ANTIALIAS)  # adjust size of images
        self.photoimg_br3=ImageTk.PhotoImage(img_browse)
        self.btn_2.config(image=self.photoimg_br3)


    def open_img_3(self):
        fln2=filedialog.askopenfilename(initialdir=os.getcwd(),title="open Images",filetypes=(("JPG File","*.jpg"),("PNG File","*.png"),("All files","*.*")))
        img_2=Image.open(fln2)
        img_browse_2=img_2.resize((540,160),Image.ANTIALIAS)  # adjust size of images
